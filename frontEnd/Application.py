...
class UserPreferenceDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        super(UserPreferenceDialog, self).__init__(parent)
...
def main(args):
    ...
    appView = Application()
    appView.hide()  # Initially hide the main eSim window
    ...
    # Function to show preferences dialog
    def show_preferences():
        ...
        if not user_preferences.get("remember_choice", False):
            ...
            if dialog.exec_() == QtWidgets.QDialog.Accepted:
                ...
                if not preferences["track_session"]:
                    print("Session tracking disabled.")
            else:
                print("User cancelled. Exiting application.")
                sys.exit(0)  # Exit if preferences dialog is cancelled

    # Callback after workspace selection to handle main window and preferences
    def after_workspace_selection():
        appView.splash.close()  # Close the splash screen
        appView.show()  # Show the main application window

        # Show preferences dialog after the main window is visible
        QtCore.QTimer.singleShot(100, show_preferences)

    if work != 0:
        appView.obj_workspace.defaultWorkspace()
        after_workspace_selection()  # Proceed to main window and preferences
    else:
        def on_workspace_closed():
            appView.obj_workspace.close()  # Ensure workspace dialog closes
            after_workspace_selection()

        # Show workspace dialog and connect its closure to the next steps
